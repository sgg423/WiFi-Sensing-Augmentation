...
- `HAR-1/CSI/M1_rf_original/user*.mat` is the RF-Diffusion input format:
  `feature` is `[num_packets x 90]` complex CSI and `cond` is a `uint8`
  metadata vector shaped `[1 x 4]`.
- The converter does not slice files into 512-packet windows. RF-Diffusion's
  loader/preprocessing handles the 512-sample time length.
- `M1_rf_original` is not a Widar `.BVP` dataset. Widar `.BVP` requires a
  separate CSI-to-BVP/velocity-spectrum preprocessing step.
...
`--rf-original`, `--pattern`, `--classes`, `--target-bins`, and `--start-index`.
...
### 5. Convert to RF-Diffusion WiFi Shape

The RF-Diffusion WiFi examples use this MAT layout:
...
Create that format with `--rf-diffusion` or `--rf-original`. This mode writes
one `user*.mat` file per PCAP file, keeps the full packet-time length for each
capture, reduces the 242 Nexmon CSI subcarriers to 90 feature bins, and writes
`cond` as `[activity, day_index, device_id, subject_id]`. It does not split
data into 512-packet windows.

For the A-F activity subset:
...
The four `cond` fields are inferred from the filename:

```text
A_1_M1_P1.pcap -> cond [1 1 1 1]
B_1_M1_P1.pcap -> cond [2 1 1 1]
F_2_M1_P3.pcap -> cond [6 2 1 3]
```
...
with `feature [T x 90]` and `cond [1 x 4] uint8`.

RF-Diffusion can then apply its own time-axis preprocessing, such as selecting
or sampling 512 time steps, inside the training/evaluation pipeline.
