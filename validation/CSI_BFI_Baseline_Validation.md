...
특정 참여자의 전체 데이터를 테스트로 제외하는 participant-independent 평가에서는 정확도가 크게 낮아졌다. 세 fold 모두 random seed 111, class weight `balanced`, normalization `none`을 사용하였다.

| Test participant | Train | Validation | Test | Accuracy | Macro-F1 | Macro-recall |
|---|---:|---:|---:|---:|---:|---:|
| P1 | 21,723 | 2,397 | 16,555 | 10.27% | 8.33% | 9.06% |
| P2 | 26,145 | 2,888 | 11,642 | 10.25% | 8.34% | 11.27% |
| P3 | 25,388 | 2,809 | 12,478 | 13.64% | 11.79% | 13.17% |
| Fold 평균 | - | - | - | **11.39%** | **9.49%** | **11.17%** |

BFI의 in-domain 정확도는 93.82%였으나 미관측 참여자 평균은 11.39%로 82.43%p 감소하였다. 따라서 현재 BeamSense CNN은 동일 데이터 분포에서는 높은 정확도를 보이지만 미관측 참여자에 대한 일반화에는 한계가 있다. 이 결과를 RF-Diffusion 증강 전 BFI cross-participant 기준선으로 사용한다.
...
#### Cross-participant 측정 결과

한 참여자의 모든 window를 학습에서 제외하고 해당 참여자만 테스트하는 participant-holdout 평가를 수행하였다. 모델, 정규화와 seed는 위 ResNet18 실험과 동일하며, 정규화 범위는 각 fold의 학습 참여자에서만 계산하였다.

| Test participant | Train | Validation | Test | Accuracy | Macro-F1 | Macro-recall |
|---|---:|---:|---:|---:|---:|---:|
| P1 | 24,101 | 2,678 | 14,473 | 12.54% | 9.88% | 13.14% |
| P2 | 25,479 | 2,832 | 12,941 | 16.33% | 12.02% | 15.54% |
| P3 | 24,672 | 2,742 | 13,838 | 12.68% | 8.08% | 12.68% |
| Fold 평균 | - | - | - | **13.85%** | **9.99%** | **13.79%** |

20개 클래스의 무작위 추측 정확도는 5%이다. CSI의 in-domain 정확도는 96.98%였으나 미관측 참여자 평균은 13.85%로 83.13%p 감소하였다. 이 결과는 현재 모델이 동일 데이터 분포에서는 행동을 잘 분류하지만 참여자 변화에 매우 민감함을 보여주며, RF-Diffusion 증강 전 CSI cross-participant 기준선으로 사용한다.
...
### 증강 전 cross-participant 기준선 요약

| 데이터 | P1 holdout | P2 holdout | P3 holdout | Fold 평균 |
|---|---:|---:|---:|---:|
| CSI · SenseFi ResNet18 | 12.54% | 16.33% | 12.68% | **13.85%** |
| BFI · BeamSense CNN | 10.27% | 10.25% | 13.64% | **11.39%** |

향후 각 fold에서 테스트 참여자를 제외한 source participant만으로 RF-Diffusion을 학습하고, 실제 source 데이터에 합성 데이터를 추가하여 같은 실제 holdout test set에서 재평가한다. 위 표는 그 증강 효과를 계산하기 위한 real-only 기준선이다.
