...
결합한다. BeamDiff는 CSI-BFI-HAR 데이터셋에서 제공하는 HAR-1 Kitchen과
HAR-3 Classroom 시나리오의 BFI 측정 데이터를 사용하여 평가하였다. 실제
validation 및 test 데이터는 고정하고 train 데이터만 1:1로 증강하였다.
HAR-1에서는 5개 모델 초기화 seed에 대한 BeamSense 평균
정확도가 92.46\%에서 96.08\%로 향상되어 평균 3.63\%p의 paired gain을
보였으며, 5개 seed 모두에서 성능이 향상되었다. HAR-3의 paired
