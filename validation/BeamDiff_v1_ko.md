...
동일한 random-window split(`split_seed=111`)과 실제 validation/test fold를 고정하고,
BeamSense 초기화 seed 42, 111, 2026, 3407, 7777에 대해 합성 데이터 비율만
변경하였다. 합성 데이터는 `augmentation_seed=111`의 중첩된 seeded prefix로
선택하여 낮은 비율의 샘플이 높은 비율에도 포함되도록 하였다.

| 합성 비율 | 합성 window | 정확도 평균 +/- 표본 SD | Baseline 대비 | 향상 seed |
|---:|---:|---:|---:|---:|
...
최신 1:1 결과는 이전에 별도로 집계했던 91.6245%에서 94.6268%로의 결과를
대체한다. 공통 ratio-sweep protocol에서는 모든 증강 비율의 평균 정확도가
baseline보다 높았으며, 75%와 100%에서는 5개 seed 모두 향상되었다. 측정한
0--100% 범위에서는 합성 비율이 증가할수록 평균 정확도가 단조 증가했고 1:1에서
가장 높은 성능을 보였다. 다만 100%를 초과한 비율에서도 계속 향상된다는 의미는
아니다.
