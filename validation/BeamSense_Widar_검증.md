...
|---|---:|---:|---:|---:|---:|---:|
| LeNet 계열 CNN | 28,876 | 6,188 | 6,188 | 85.13% | 84.97% | 85.12% |
| ResNet18 계열 CNN | 28,876 | 6,188 | 6,188 | **95.10%** | **95.20%** | **95.30%** |

이를 통해 HAR-1 단일 링크 CSI 진폭에도 20개 행동을 구분할 수 있는 센싱 정보가 존재함을 확인하였다. 다만 사후 코드 대조 결과 이 예비 실험의 세부 계층은 SenseFi 공식 UT-HAR 구현과 달랐으므로, 공식 구조로 재측정하기 전에는 SenseFi 재현 결과로 사용하지 않는다.

### CSI·BFI 기준 성능 요약

| 데이터 | 센싱 표현 | 공개 모델 기반 분류기 | Accuracy | Macro-F1 | Macro-recall |
|---|---|---|---:|---:|---:|
| CSI | 진폭 `(1, 250, 90)` | ResNet18 계열 예비 모델 | 95.10% | 95.20% | 95.30% |
| BFI | BFA `(10, 234, 4)` | BeamSense CNN | 93.82% | 94.07% | 94.13% |

두 결과는 각각의 공개 모델을 이용한 독립적인 기준 성능이다. 입력 표현과 분류기가 다르므로 95.10%와 93.82%의 차이를 CSI와 BFI 자체의 우열로 해석하지 않는다.
