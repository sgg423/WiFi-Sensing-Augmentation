...
BeamDiff는 CSI-BFI-HAR 데이터셋에서 제공하는 HAR-1 Kitchen과 HAR-3
Classroom 시나리오의 BFI 측정 데이터를 사용하여 평가하였다
\cite{haque2026datasets}. 모든 실험은 real-only와 real-plus-synthetic
조건을 paired protocol로 비교했으며, 실제
validation 및 test split은 동일하게 고정하고 train split에만 합성 데이터를
추가하였다. 이를 통해 생성 데이터의 fidelity, downstream 센싱 성능 및 모델
초기화 seed에 따른 강건성을 평가한다.
...
\begin{itemize}
    ...
    증강 이득, 생성 데이터 fidelity 및 모델 seed에 따른 변화를 분석한다.
\end{itemize}

% 최종 제출 전 모든 BibTeX key를 확인하고, 구체적인 수치 결과는 초록과
% 평가 절에서만 제시한다.
