...
localization, respiration monitoring, gesture recognition, and human activity
recognition \cite{wang2015understanding}.
...
We evaluate BeamDiff on the HAR-1 and HAR-3 environments of the CSI-BFI-HAR
dataset using paired real-only and real-plus-synthetic training protocols. The
real validation and test folds remain fixed, and only training data are augmented.
On HAR-1, one-to-one augmentation improves the mean BeamSense accuracy from
91.62\% to 94.63\% across five model initialization seeds, corresponding to a
mean paired gain of 3.00 percentage points, with improvements in four of five
seeds. On HAR-3, an initial paired evaluation improves accuracy from 98.70\% to
99.65\% and reduces the number of test errors from 81 to 22. These results show
that BFA-specific generation can improve in-domain sensing performance, while
cross-participant experiments reveal that generalization to unseen participants
remains a separate and substantially harder problem.
...
\begin{itemize}
    ...
    environments and analyze augmentation gain, generated-data fidelity,
    seed-dependent behavior, and the remaining limitations under
    cross-participant evaluation.
\end{itemize}

% Before final submission, replace the single-seed HAR-3 sentence with the
% completed multi-seed mean and variability, and verify all BibTeX keys.
